...
all: version boot kernel libc $(LINKER_SCRIPT)
	...
	dd conv=notrunc seek=170 if=tools/mkfs/disk.pup of=$(DISK_NAME)
	#dd conv=notrunc seek=170 if=test/exp/exp.out of=$(DISK_NAME)
	#dd conv=notrunc seek=210 if=test/derp/derp.out of=$(DISK_NAME)
