...
void k_putchar(char c)
{
    ...
    else if(c == 0x10)
    {
        ...
        else
        {
            ...
            {
                vga_cur_row--;
                vga_cur_column = k_gfx_vga_width - 1;
            }
        }

        uint16_t entry = k_vga_char(' ', vga_cur_color);
        k_gfx_vga_cur_mem_map[vga_cur_column + vga_cur_row * k_gfx_vga_width] = entry;
    }
    else
    {
        ...
        vga_cur_column++;
        if(vga_cur_column >= k_gfx_vga_width)
        {
            vga_cur_column = 0;
            vga_cur_row++;
        }
    ...
    if(vga_cur_row >= k_gfx_vga_height)
    {
        vga_cur_row = 0;
    }
}
...
void k_term_clear()
{
    ...
    for(uint32_t row = 0; row < k_gfx_vga_height; row++)
    {
        uint32_t offset = row * k_gfx_vga_width;

        for(uint32_t column = 0; column < k_gfx_vga_width; column++)
        {
