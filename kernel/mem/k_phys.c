...
extern struct k_mem_plist_t k_mem_physical_pages;
    ...
    if(k_mem_physical_pages.free_pages_count && k_mem_physical_pages.free_pages_count >= page_count)
    {
        ...
        for(size_t entry_index = 0; entry_index <= entry_count; entry_index++)
        {
            ...
            if(head_entry)
            {
                ...
                k_mem_physical_pages.free_pages_count -= page_count;
...
uint32_t k_mem_IsValidPage(uintptr_t page_address)
{
    ...
    {
        return k_mem_physical_pages.used_pages[K_MEM_USED_SMALL_PAGE_ENTRY_INDEX(page_address)].pid_index != K_MEM_USED_SMALL_PAGE_INVALID_ENTRY_INDEX;
    }
    ...
    if(page_address != K_MEM_INVALID_PAGE)
    {
        ...
        if((used_entry->flags & (K_MEM_PAGE_FLAG_USED | K_MEM_PAGE_FLAG_HEAD)) == (K_MEM_PAGE_FLAG_USED | K_MEM_PAGE_FLAG_HEAD))
        {
            do
            {
                ...
                used_entry = k_mem_physical_pages.used_pages + entry_index;
            }
            while(entry_index <= k_mem_physical_pages.last_used_page &&
