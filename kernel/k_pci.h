...
struct k_pci_header_t
{
    ...
    union
    {
        ...
        struct
        {
            uint32_t base_address_regs0[6];
            uint32_t cardbus_cis_pointer;
            uint16_t subsystem_vendor_id;
            uint16_t subsystem_id;
            uint32_t exp_rom_base_address;
            uint8_t reserved1[3];
            uint8_t cap_pointer0;
            uint32_t reserved2;
            uint8_t max_lat;
            uint8_t min_gnt;
            uint8_t interrupt_pin0;
            uint8_t interrupt_line0;
        };

        /* type 1 header */
        struct
        {
            uint32_t base_address_regs1[2];
